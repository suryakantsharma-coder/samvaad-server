 ...
 */
export async function extractAppointmentFromTranscript(
  input: AppointmentExtractionInput,
): Promise<AppointmentExtractionOutput> {
  ...
  try {
    const parsed = JSON.parse(rawText) as AppointmentExtractionOutput;
    // Basic sanity checks
    ...
    // Log the bad payload to aid debugging
