...
// const TRANSCRIPT_ONLY_SILENCE_MS = 100; // very short — reply in ~1s after user stops
// const TRANSCRIPT_ONLY_SPEECH_THRESHOLD = 220; // detect speech early
// const TRANSCRIPT_ONLY_MIN_DURATION_MS = 120; // send quickly, minimal buffer
...
//     // 2) ANSWER FAST: As soon as you have the user's input, respond immediately. No "one moment", no filler, no delay. Listen fast, answer fast.
//     // 3) Speak short and clear — one thing at a time. Brief pause (half second max) between question and next. No long silences.
...
//     // PATIENT NAME RULE (TEXT ONLY — NO SPELLING):
//     // - Ask for FULL name only in callerLanguage. Examples: HI — "पूरा नाम बताइए?" / "नाम?"; GU — "પૂરું નામ જણાવશો?" / "નામ?" — no spelling, no letter-by-letter.
//     // - Caller will say the name in their words (Hindi/Gujarati/any). You convert it to English (Roman) silently for create_patient. Example: "राजेश कुमार" → Rajesh Kumar; "સીતા પટેલ" → Sita Patel.
//     // - After they say the name, confirm in ONE short sentence in callerLanguage: "नाम [NAME in English] है, सही?" (HI) / "નામ [NAME in English] છે, સાચું?" (GU). If they say NO, ask again and re-confirm.
//     // - Never say you are converting or spelling — just confirm the name and use English fullName in create_patient.
//     // - Only after name confirmation, call create_patient with fullName in English only. Then confirm patientId to caller. For appointment use patient._id (NOT patientId).
...
//     // - Always call list_doctors before booking.
//     // - Always save reason in English only.
//     // - Answer immediately after user speaks — no buffer, no delay. Fast listen, fast reply.
//     // - Never say you are "converting" the name to English — use the English name silently; do not announce conversion to the caller.
...
//         // Stream output audio to Exotel: send each chunk immediately (no buffering)
//         if (
...
//               if (userIsSpeaking) break; // drop remaining chunks if user started speaking
...
//         ) {
//           console.log(
//             "[Agent] Input transcription (Whisper):",
...
//                 .catch((err) => {
//                   console.error(
//                     "[Agent] Whisper transcription error:",
//                     err.message,
//                   );
//                 })
//                 .finally(() => {
